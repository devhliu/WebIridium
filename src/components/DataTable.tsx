...
export interface DataTableProps {
  ...
  /* default: false */
  firstColumnIsIndependent?: boolean;
}

const DataTable = memo(
  ({ columns, decimalPlaces, firstColumnIsIndependent }: DataTableProps) => {
    if (columns.length === 0) {
      return null;
    }

    const allNumericValues = columns
      .filter((_, index) => !firstColumnIsIndependent || index > 0)
      .map((c) => c.values)
      .flat()
      .map(Number)
      .filter((v) => !isNaN(v));
              ...
              <tr key={rowIndex} className={styles.row}>
                {columns.map((column, colIndex) => {
                  ...
                  if (
                    ...
                    (!firstColumnIsIndependent || colIndex > 0)
                  ) {
                    const targetColor =
                      cellValue >= 0
